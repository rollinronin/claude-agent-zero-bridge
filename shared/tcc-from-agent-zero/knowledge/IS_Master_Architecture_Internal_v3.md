...
Master Reference Document — Updated March 27, 2026
...
| **Tag** | **IS PMO #** | **Project** | **IS role** | **Key constraint** |
| --- | --- | --- | --- | --- |
| EXP | **222** | TCC Expansion | Lead — deliver PC refresh for 30 consoles (60 IS PCs: Council + LENS networks). Solacom PCs managed by IES — not IS. Initial phase: 16 PCs for EBC temp space (Room 1007 at OMF). | Construction schedule drives all IS milestones. NTP pending. ~921 tasks in reconciled WBS. |
| NG911 | **193** | Solacom NG911 Refresh | Coordination — ensure selected solution meshes with IS infrastructure. Drive: new vendor owns 100% end-to-end support, eliminating POTS/DID lines between IS-managed Mitel and Solacom. Direction confirmed: cloud-hosted managed solution only. | IS must fully disengage from Mitel-to-Solacom support. This requirement must be written into the RFP at the deep-dive workshop and survive contract negotiations. Multi-year timeline — FE schedule extends into 2027. |
| VUW | **11138** | VuWall Video Wall/KVM | Coordination — infrastructure readiness, CJIS/BCA approval, POC environment support, support gap assessment. | PO issued March 26, 2026. Non-refundable POC cost. IS support gap is primary risk. Bluum (reseller) contract status uncertain — parallel procurement path exploration underway. |
...
| Josh Alswager — IS Principal Specialist | NG911 | Reports directly to CIO. Former IS Network Manager; left for state PSAP work; returned with deep 911 operational knowledge. Primary Met Council technical representative in FE-led NG911 RFP process. Participant in NG911 requirements workshops. |
| John Stefanko — Director of IS Architecture | NG911, VUW | IS Architecture team lead. Team also known as the IS Triage team — reviews and evaluates IS infrastructure requests and new technology submissions before work is accepted. Initial VuWall technical review (IS PMO #11138) was conducted by Sreeni Nutulapati, who reports to Stefanko. Participant in NG911 requirements workshops. SteerCo attendee. |
| Sreeni Nutulapati — IS Architecture / Triage | VUW | Reports to John Stefanko. Conducted the initial VuWall (IS PMO #11138) technical evaluation and gave thumbs-up approval. IS Architecture team (Triage) is the front door for new technology submissions. |
| Steven Kensinger — Principal Contract Administrator, Regional Administration Procurement | NG911 | Leads the Council through the formal NG911 RFP process: advertisement, vendor evaluation, contract execution. Distinct from IS Procurement (Tina Folch) — this procurement is managed through Regional Administration because Metro Transit / TCC will own the vendor relationship, not IS. Participant in NG911 requirements workshops. |
| Lisa Belland — Business Continuity Manager | NG911, EXP | Oversees the Council's COOP (Continuity of Operations) program. Relevant to NG911 — any 911 system replacement has COOP implications and must align with Council COOP standards. Also relevant to EBC buildout (Room 1007 is a COOP facility). Participant in NG911 requirements workshops. |
...
| Bluum | VUW | State contract reseller/integrator and IS service integration partner. Introduced both Userful (first video wall solution evaluated, shut down due to network security concerns) and VuWall to TCC. IS deliberately preserved the Bluum relationship and gave them a fair shot before exploring alternatives, in recognition of the legwork they had done. Bluum's state contract came under risk of cancellation toward the end of 2025 — that's when IS began exploring SHI as an alternative. Bluum received a last-minute short extension on December 15, 2025 (3 months, expiring ~March 2026); IS PM was on Christmas vacation when it came through. IS expected a full renewal in March–April 2026 but that did not materialize, which is why IS is now actively exploring parallel procurement paths. |
| Tina Folch — IS Procurement | VUW, EXP | IS-managed infrastructure procurement: VuWall PO lead, Bluum contract contact, Room 1007 hardware procurement. Scope is IS-managed equipment only. NG911 procurement is handled separately by Steven Kensinger (Regional Administration) because TCC owns the vendor relationship. Stretched thin — parallel VuWall support partner procurement exploration may need a different owner. |
...
## **1.6  Open action items (as of March 26, 2026)**

| **Action item** | **Owner** | **Status** |
| --- | --- | --- |
| Initiate PC procurement — 3 eval units first (2 Council/special build + 1 LENS/catalog), then full order of 16 | IS PM + Mark Linnell | This week — URGENT |
| Paul Burke meeting — full IS infrastructure scope alignment (network, security, voice, endpoint, server/identity) for TCC and VuWall | IS PM + Paul Burke | March 27 |
| FE NG911 weekly — confirm status, schedule deep-dive RFP workshop | IS PM + Josh Alswager | March 27 |
...
| Schedule Eric Brown (LASO) intro — after Paul Burke meeting when design detail is available | IS PM + Tracy French | After March 27 |
...
## **4.1  SharePoint folder structure**

Build this structure first. Use naming convention: YYYY-MM-DD_[Tag]_[DocType]_[Version].ext

| TCC IS Projects (SharePoint site root)   ├── _Portfolio Hub   │   ├── Bridge Documents   │   ├── Integrated Plans   │   └── Executive Briefings   ├── TCC Expansion (EXP)   │   ├── Project Charter & Scope   │   ├── WBS & Schedule   │   ├── Technical Documentation   │   ├── Procurement   │   ├── Meeting Notes   │   └── Status Reports   ├── NG911 Refresh   │   ├── FE Coordination   │   ├── RFP Documents   │   ├── IS Requirements   │   ├── Meeting Notes   │   └── Status Reports   └── VuWall       ├── Vendor Documentation       ├── POC Planning       ├── IS Requirements       └── Meeting Notes |
| --- |
...
1. IS and IS infrastructure team (Paul Burke) produce formal design documentation describing the proposed architecture, network configuration, encryption approach, authentication mechanisms, key management and rotation, and data handling at rest and in transit.
2. Eric Brown (LASO — Local Agency Security Officer) reviews the design and prepares a formal design proposal submission to the BCA (Bureau of Criminal Apprehension), which is Minnesota's CJIS Systems Agency.
3. BCA reviews and certifies the design. This is the gate for IS to proceed with implementation.
4. LASO maintains ongoing security documentation, ensures personnel screening (fingerprint-based background checks required before CJI access), and reports security incidents to the BCA.
...
| --- |
| You are helping me manage the TCC Expansion IS project (IS PMO #222) as part of a portfolio |
...
| --- |
| You are helping me manage IS involvement in the Solacom NG911 System Refresh (IS PMO #193) |
...
|   left for state PSAP work; returned with deep 911 operational knowledge. Primary |
|   Met Council technical representative in the FE-led process. Participant in |
|   NG911 requirements workshops. |
| - Steven Kensinger: Principal Contract Administrator, Regional Administration Procurement. |
|   Leads the Council through the formal RFP process (advertisement, vendor evaluation, |
|   contract execution). This is NOT IS Procurement — TCC / Metro Transit will own the |
|   vendor relationship. Kensinger's involvement reflects the scale and cross-organizational |
|   nature of this procurement. Participant in NG911 requirements workshops. |
| - Lisa Belland: Business Continuity Manager. Oversees Council COOP program. |
|   Any 911 system replacement must align with Council COOP standards and continuity |
|   requirements. Participant in NG911 requirements workshops. |
| - John Stefanko: Director of IS Architecture (IS Triage team lead). Participant in |
|   NG911 requirements workshops. Architecture review of NG911 solution is within his |
|   team's scope. |
...
| - IS PM (me): infrastructure oversight; monitor IS impacts; IS 24/7 disengagement requirement |
...
| --- |
| You are helping me manage IS involvement in the VuWall Video Wall / KVM project (IS PMO #11138) |
| at the Transit Control Center (TCC), Metro Transit, Twin Cities Metropolitan Council. |
| |
| ROLE: Senior PM, PMO, Infrastructure Services. Locked-down government work PC; |
| personal PC has full Claude Desktop + M365 desktop apps. |
...
| - John Stefanko: Director of IS Architecture — his team (IS Triage / Architecture) did the |
|   initial VuWall (IS PMO #11138) technical evaluation. Sreeni Nutulapati (IS Architecture, |
|   reports to Stefanko) conducted the review and gave initial approval. |
| - Tina Folch: IS Procurement — PO lead, Bluum contact (scope: IS-managed equipment only) |
| - Bluum: state contract reseller through which VuWall equipment is procured |
| |
| PROJECT HISTORY (context for AI — do not include in external-facing documents): |
| |
| PRIOR SOLUTION — USERFUL: |
| VuWall was the second video wall solution evaluated for TCC. The first was Userful, |
| evaluated before IS PM was engaged in the project. Bluum (IS service integration |
| partner and state contract reseller) introduced Userful to TCC. That evaluation was |
| shut down due to network security concerns. Bluum then introduced VuWall as an |
| alternative. |
| |
| BLUUM RELATIONSHIP — WHY IS PRESERVED IT: |
| Bluum had done legwork for IS on the VuWall opportunity. IS made a deliberate decision |
| to give Bluum a fair shot and not immediately pivot to exploring SHI or other |
| alternatives — in recognition of that investment in the relationship. |
| The situation became complicated when Bluum's state contract came under risk of |
| cancellation toward the end of 2025. That's when IS began exploring SHI as a parallel |
| procurement path. At the last minute, Bluum received a short contract extension on |
| December 15, 2025 (3 months, expiring ~March 2026) — IS PM was on Christmas vacation |
| when it came through. IS expected a full renewal in March–April 2026, but that renewal |
| did not materialize. That is why IS is now actively pursuing parallel procurement paths |
| (Bluum renewal, SHI on state contract, cooperative purchasing vehicles) rather than |
| depending solely on Bluum. |
| |
| IS PMO INTAKE — SERVICE TICKET 11138: |
| IS PM's first activity on this project was to gather all VuWall technical documentation |
| and submit a formal IS PMO service request under the ServiceWorks ITSM system. |
| The ticket number is 11138 — this is both the IS PMO project number and the original |
| technical assessment ticket. Context: IS PMO was updating its intake process at the |
| same time the organization was implementing ServiceWorks as a new ITSM platform. |
| The decision was made to use the ServiceWorks ticket directly as the project number |
| rather than a separate tracking system. Sreeni Nutulapati (IS Architecture, reports |
| to John Stefanko) conducted the technical evaluation under ticket 11138 and gave |
| a thumbs-up approval. This confirmed VuWall as a viable solution before any |
| commercial relationship was established. |
| |
| PROCUREMENT GOVERNANCE — WHY BCA ENGAGEMENT WAS DELIBERATELY DEFERRED: |
| After the technical assessment, there was a question (raised by Gretchen White / IS |
| Director) about why BCA/CJIS engagement (Eric Brown, LASO) had not yet begun. |
| IS PM's deliberate decision was to hold deep technical engagement — including BCA |
| design work and IS infrastructure discussions with the vendor — until the commercial |
| relationship was confirmed via PO. Rationale: |
| 1. The Bluum contract situation was tenuous (short renewal window, competitive bid |
|    a real possibility). If IS had to go to competitive bid, early deep engagement |
|    with VuWall could invalidate the vendor selection process or compromise |
|    bid integrity by sharing IS infrastructure specifics with a vendor not yet |
|    under contract. |
| 2. BCA design proposals require specific IS infrastructure detail. Producing that |
|    documentation requires deep vendor collaboration. That collaboration should not |
|    happen before a confirmed commercial relationship. |
| 3. IS PM already had sufficient technical confidence (ticket 11138 thumbs-up) |
|    to proceed with the business case and procurement. BCA engagement could wait |
|    for PO confirmation without meaningful schedule risk. |
| This was a sound procurement governance call, not a delay. With PO confirmed |
| (March 26, 2026), full technical engagement begins immediately — including |
| BCA/CJIS design work, IS infrastructure planning, and VuWall vendor deep-dive. |
| |
| PROCESS NOTE: |
| This project originated from Bluum (IS service integration partner) introducing |
| first Userful, then VuWall, to TCC — rather than IS Architecture / Triage |
| identifying the requirement and evaluating solutions independently. This is the |
| pattern IS is working to move away from. The preferred model is for the business |
| to bring a requirement to IS, and IS to identify and evaluate solutions. This is |
| not a criticism of how the project started; it reflects the operational reality |
| IS PM inherited and is managing forward from where it is. |
| |
| DATA ARCHITECTURE: SharePoint Lists master data; Planner for active task execution. |
| |
...
| 1. TCC Expansion (EXP, IS PMO #222) — IS lead. PC refresh for 30 consoles: 2 IS PCs each |
...
| 2. Solacom NG911 Refresh (NG911, IS PMO #193) — IS coordination. FE leads RFP. Direction: |
...
| 3. VuWall (VUW, IS PMO #11138) — IS coordination. Video wall/KVM. PO issued March 26. |
...
| - John Stefanko: Director of IS Architecture (IS Triage team lead) — SteerCo attendee, |
|   NG911 and VuWall architecture review |
| - Steven Kensinger: Principal Contract Administrator, Regional Administration Procurement — |
|   leads NG911 RFP formal process; TCC owns vendor relationship, not IS |
| - Lisa Belland: Business Continuity Manager — COOP program; NG911 requirements workshops |
